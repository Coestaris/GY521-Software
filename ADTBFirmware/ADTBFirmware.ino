...
void loop() {
	...
	else if (cmd == 2)
	{
		...
		uint32_t crc = crc32(data, 14);
		data[14] = (byte)(crc & 0xFF);
		data[15] = (byte)((crc >> 8) & 0xFF);
		data[16] = (byte)((crc >> 16) & 0xFF);
		data[17] = (byte)((crc >> 24) & 0xFF);


		Serial.write(0x0); //Dummy bytes
		Serial.write(0x0); //Dummy bytes
...
uint32_t crc32(uint8_t bytes[18], uint32_t len)
{
